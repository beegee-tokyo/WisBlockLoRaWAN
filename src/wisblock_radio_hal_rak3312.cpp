...
/*
 * RAK3312 (ESP32-S3) implementation. Same sx126x_hal_* contract as
 * wisblock_radio_hal_rak4631.cpp — only the SPI init call and a couple of
 * ESP32-specific quirks (IRAM-safe busy-wait, VSPI/FSPI peripheral choice)
 * differ underneath.
 */
...
// it never sends. This library never explicitly puts the radio to sleep
// itself, but LBM's own radio_planner does (ral_set_sleep(), see
// src/lbm/smtc_modem_core/radio_planner/src/radio_planner.c), autonomously,
// between scheduled tasks - so this HAL must track that state regardless of
// what higher-level code in this library does or doesn't do.
...
void checkDeviceReady()
{
	...
	// Re-enable the SPI peripheral disabled in the SetSleep branch above -
...
void init()
{
	...
	// doesn't span.
	pinMode(LORA_SPI_NSS, OUTPUT);
	digitalWrite(LORA_SPI_NSS, HIGH);
...
extern "C"
{
	...
	{
		const WisBlockRadioContext *ctx = &wisblockRadioContext;
		(void)context; // NOTE: LBM instantiates its radio as RALF_SX126X_INSTANTIATE(NULL)
		...
		digitalWrite(ctx->pinReset, LOW);
		delayMicroseconds(kResetPulseUs);
		digitalWrite(ctx->pinReset, HIGH);
		radioMode = RadioMode::Awake;
